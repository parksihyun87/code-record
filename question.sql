...
select bookname,publisher,price from booktbl where publisher not in ('굿스포츠','대한미디어');
...
-- ⑦도서이름이 여섯글자인 도서를 검색
select bookname,publisher from booktbl where bookname like '______';
...
select bookname,publisher,price from booktbl where price>=20000;
...
select bookname,publisher,price from booktbl order by price desc,publisher ;
-- 기본적으로 a 오름차순임
...
select sum(saleprice) from orderstbl where custid=2;
-- 16) 고객이 주문한 도서의 총 판매액, 평균값, 최저가, 최고가 조회
select sum(saleprice),avg(saleprice),min(saleprice),max(saleprice) from orderstbl;
-- 17) 마당서점의 도서 판매 건수 조회
select count(*) from orderstbl;
-- 18) 고객별로 주문한 도서의 총 수량과 총 판매액 조회
select custid,count(*),sum(saleprice) from orderstbl group by custid;
-- 19) 가격이 8,000원 이상인 도서를 구매한 고객에 대하여 고객별 주문 도서의 총 수량을 구하시오. 단, 두 권 이상 구매한 고객만 조회 count(*)>=2 or
select custid,count(*) from orderstbl where saleprice>=8000 group by custid having count(*)>=2;
